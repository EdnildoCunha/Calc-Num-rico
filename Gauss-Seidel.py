...
def seidel(M, erro, iteracao):
    '''
    Entradas: matriz aumentada, erro, e quantidade de iterações. 
    Saídas: Vetor solução do sistema, quantidade de iteração ou mensagem de erro'''
    n = len(M)
    on = True
    cont = 0
    # Extrair o vetor b
    ...
    while on == True:
        ...
        if d<erro or cont==iteracao:
            on = False
            print("Vetor Solução do Sistema:")
            print(x1)
